...
# Filter rows with occupancy >= 30 and <= 88
gene_counts <- gene_counts %>%
  filter(Occupancy >= 30 & Occupancy <= 88)
...
tmp_loss_dir <- "/scratch4/agordus1/crunnel2/tmp_loss"
if (!dir.exists(tmp_loss_dir)) dir.create(tmp_loss_dir, recursive = TRUE)
...
# Progress file setup
progress_file <- "/scratch4/agordus1/crunnel2/phyloglm_progress_loss.txt"
...
loss_time <- system.time({
  foreach(g = unique_genes, .packages = c("phylolm", "phytools", "ape")) %dopar% {
    ...
    tmpfile <- file.path(tmp_loss_dir, paste0(g, ".rds"))
    saveRDS(out, tmpfile)
...
cat("Finished parallelized phyloglm regressions for gene loss.\n")

# Combine temp files into final CSV
combine_tmp_results(tmp_loss_dir, "/home/crunnel2/orb-selection/results/phyloglm_loss.csv")

# Calculate Storey FDR (q-values) and save updated CSV
library(qvalue)
results <- read.csv("/home/crunnel2/orb-selection/results/phyloglm_loss.csv")
if ("coef_orb_weavingTRUE_p.value" %in% colnames(results)) {
  pvals <- results$coef_orb_weavingTRUE_p.value
  qobj <- qvalue(p = pvals)
  results$qvalue <- qobj$qvalues
  write.csv(results, "/home/crunnel2/orb-selection/results/phyloglm_loss_qvals.csv", row.names = FALSE)
} else {
