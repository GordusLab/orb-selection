...
differs significantly between orb-weavers and non-orb-weavers
"""
...
import id_converter as id_converter
from orthogroup_filter import drop_empty_cols
...
class OddsRatioResults:
    ...
    def filter_for_bootstrap_hits(
        self, minimum, maximum, ci, alternative="two-tailed"
    ):
...
#        stat,
        ...
        if alternative not in alts:
            raise ValueError("Invalid alternative hypothesis. Expected one of: %s" % alts)
...
#            f"stat={self.stat}, "
    ...
    def _run_bootstrap(self):
        ...
        if self.alternative == "greater":
            print("Counting bootstrapped distributions in which statistics\n"
    ...
    def plot_bootstrap_results(
        ...
        if self.alternative == "greater":
            alpha_threshold = self.ci_av[1]
            alt="right-tailed"
        else:
            alpha_threshold = self.ci_av[0]
            alt="left-tailed"
