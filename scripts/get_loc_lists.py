...
def get_universe_bootstrap(bs_results) -> pd.Series:
    ...
    genecount_df['occupancy'] = genecount_df.select_dtypes(include='number').astype('bool').sum(axis=1)

    # Drop all columns except 'HOG' and 'occupancy'
    genecount_df = genecount_df[['occupancy']]

    if max_occupancy is not None:
        genecount_df = genecount_df[(genecount_df['occupancy'] >= min_occupancy) & (genecount_df['occupancy'] <= max_occupancy)]
    else:
...
def main_hyphy(hyphy_results, omega=10000, relax_result=None):
    ...
    hit_locs = get_fltrd_LOCs(merged_df, omega=omega, test=test, relax_result=relax_result)
...
Examples:
  ...
  python get_loc_lists.py results.pkl --tail right --hits-file locs.txt
    ...
    parser.add_argument('--hits-file', dest='hits_file',
                    help='Save results to file instead of printing to console')
    parser.add_argument('--universe-file', dest='universe_file',
